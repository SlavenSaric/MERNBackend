...
const signup = async (req, res, next) => {
  ...
  let token
  try{
    token = jwt.sign({userId: createdUser.userId, email: createdUser.email}, 'supersecret_dont_share', {expiresIn: '1h'})
  }catch(err){
    const error = new HttpError("Signing up failed. Please try again", 500);
    return next(error);
  }
...
const login = async (req, res, next) => {
  ...
  let token
  try{
    token = jwt.sign({userId: existingUser.userId, email: existingUser.email}, 'supersecret_dont_share', {expiresIn: '1h'})
  }catch(err){
